...
class ResNetMulti(nn.Module):
    ...
    def forward(self, cf, kf, flow, device, source='src'):
        ...
        # kf_aux_rec = np.zeros(kf_aux.shape)
        # rec_positions_aux = np.zeros(kf_aux.shape)
        # for x in range(kf_aux.shape[-1]):
        #     for y in range(kf_aux.shape[-2]):
        ...
        #             rec_positions_aux[:, :, y_flow, x_flow] = 1
        # kf_aux_rec = torch.from_numpy(kf_aux_rec).float().cuda(device)
        # rec_positions_aux = torch.from_numpy(rec_positions_aux).float().cuda(device)
        ...
        # pred_aux = self.sf_layer(torch.cat((cf_aux, rec_positions_aux*kf_aux_rec), dim=1))
